...
const statusColumns: { key: DisplayStatus; label: string; dotColor: string }[] = [
  ...
  { key: "atribuido", label: "Atribu├¡do", dotColor: displayStatusDotColor.atribuido },
  { key: "pendencias", label: "Vistoria com Pend├¬ncia", dotColor: displayStatusDotColor.pendencias },
  { key: "certificado_termo", label: "Certificado Provis├│rio", dotColor: displayStatusDotColor.certificado_termo },
    ...
    const fetchData = async () => {
        ...
        const mockProcs: ProcessoWithProtocolo[] = [
          {
            ...
            protocolos: {
              ...
              razao_social: "Com├®rcio de Alimentos Silva Ltda",
              cnpj: "12345678000190",
          ...
          },
          {
            ...
            protocolos: {
              ...
              nome_fantasia: null,
              razao_social: "Posto de Combust├¡veis Acreano",
              cnpj: "98765432000110",
      ...
      const mapped: ProcessoWithProtocolo[] = (procs || []).map((p: any) => {
        ...
        return {
          ...
          vistoriador_nome: profMap[activeVistoriadorId || ""] || "N├úo atribu├¡do",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Fluxo autom├ítico de processos ÔÇö agrupados por regional
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              placeholder="Buscar por n┬║, raz├úo social, CNPJ, munic├¡pio..."
                                ...
                                <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground mb-2">
                                  ...
                                  <span>
                                    {process.data_2_retorno ? (
                                      `2┬║ Retorno: ${new Date(process.data_2_retorno + "T00:00:00").toLocaleDateString("pt-BR")}`
                                    ) : process.data_1_retorno ? (
                                      `1┬║ Retorno: ${new Date(process.data_1_retorno + "T00:00:00").toLocaleDateString("pt-BR")}`
                                    ) : (
                                      `Solicita├º├úo: ${new Date(process.data_solicitacao + "T00:00:00").toLocaleDateString("pt-BR")}`
                                    )}
                                  </span>
                                </div>
                                  ...
                                  <div className="mt-3 pt-3 border-t border-border space-y-3 animate-fade-in">
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                        ...
                                        <span>Solicita├º├úo: {new Date(process.data_solicitacao + "T00:00:00").toLocaleDateString("pt-BR")}</span>
                                      </div>

                                      {process.data_1_retorno && (
                                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                          <Calendar className="w-3.5 h-3.5 shrink-0" />
                                          <span>1┬║ Retorno: {new Date(process.data_1_retorno + "T00:00:00").toLocaleDateString("pt-BR")}</span>
                                        </div>
                                      )}
                                      ...
                                      {process.data_2_retorno && (
                                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                          <Calendar className="w-3.5 h-3.5 shrink-0" />
                                          <span>2┬║ Retorno: {new Date(process.data_2_retorno + "T00:00:00").toLocaleDateString("pt-BR")}</span>
                                        </div>
                                      )}
                                      ...
                                      {process.protocolos.area && (
                                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                          ...
                                          <span>├ürea: {process.protocolos.area}m┬▓</span>
                                        </div>
                                      )}
                                    </div>
